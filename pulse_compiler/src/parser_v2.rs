...
use pulse_core::{Constant, PulseError, PulseResult};
...
impl<'a> ParserV2<'a> {
    ...
    fn error_at(&self, message: impl Into<String>, line: usize, column: usize) -> PulseError {
    ...
    fn error(&self, message: impl Into<String>) -> PulseError {
    ...
    fn match_statement(&mut self) -> PulseResult<Stmt> {
        ...
        if self.matches(Token::Semicolon)? {
            // Do nothing - semicolon is optional here
        }
    ...
    fn send_statement(&mut self) -> PulseResult<Stmt> {
        // send(actor, message) syntax
        self.consume(Token::LeftParen, "Expect '(' after 'send'.")?;
    ...
    fn receive_statement(&mut self) -> PulseResult<Stmt> {
        ...
        self.consume(Token::Semicolon, "Expect ';' after receive.")?;
    ...
    fn print_statement(&mut self, in_match_arm: bool) -> PulseResult<Stmt> {
        ...
        if in_match_arm {
            // Don't consume any terminator - let the match_statement handle it
        } else {
            // Accept only semicolon as terminator
            self.consume(Token::Semicolon, "Expect ';' after value.")?;
        }
        if exprs.len() == 1 {
            Ok(Stmt::Print(exprs.remove(0)))
        } else {
            Ok(Stmt::Block(exprs.into_iter().map(Stmt::Print).collect()))
        }
    }
