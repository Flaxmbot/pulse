...
impl SharedHeap {
    ...
    pub fn get(&self, handle: SharedHandle) -> Option<pulse_ast::object::SharedMemory> {
    ...
    pub fn get_mut(&self, handle: SharedHandle) -> Option<pulse_ast::object::SharedMemory> {
        let mut entries = self.entries.write();
        if handle.0 < entries.len() {
    ...
    pub fn set(&self, handle: SharedHandle, value: pulse_ast::Value) -> bool {
        let mut entries = self.entries.write();
        if handle.0 < entries.len() {
...
mod tests {
    use super::*;
    use pulse_ast::object::SharedMemory;
    use pulse_ast::Value;
