use pulse_ast::object::{
    BoundMethod, Closure, Function, HeapInterface, Instance, ObjHandle, Object,
};
use pulse_ast::{ActorId, Chunk, Constant, NativeFn, Op, PulseError, PulseResult, Value};
...
impl VM {
    ...
    pub fn new_spawn(
        ...
        for val_const in captured_upvalues {
            ...
            let uv = Object::Upvalue(pulse_ast::object::Upvalue {
                location: None,
                closed: Some(val),
            });
    ...
    pub fn define_native(&mut self, name: &str, func: pulse_ast::value::SyncNativeFn) {
        let native = NativeFn {
            name: name.to_string(),
            func: pulse_ast::value::NativeFunctionKind::Sync(func),
        };
        let handle = self.heap.alloc(Object::NativeFn(native));
        self.builtins.insert(name.to_string(), Value::Obj(handle));
    }

    pub fn define_native_async(&mut self, name: &str, func: pulse_ast::value::AsyncNativeFn) {
        let native = NativeFn {
            name: name.to_string(),
            func: pulse_ast::value::NativeFunctionKind::Async(func),
        };
        let handle = self.heap.alloc(Object::NativeFn(native));
        self.builtins.insert(name.to_string(), Value::Obj(handle));
    }
        ...
        match c {
            ...
            Constant::Closure(f, upvals) => {
                ...
                for uv_const in upvals {
                    ...
                    let uv_obj = Object::Upvalue(pulse_ast::object::Upvalue {
                        location: None,
                        closed: Some(val),
                    });
            ...
            ($op:expr) => {
                ...
                match $op {
                            ...
                            Value::Obj(handle) => {
                                 ...
                                 if obj_type == 1 { // Native
                                     ...
                                     let result = match native.func {
                                         pulse_ast::value::NativeFunctionKind::Sync(f) => f(self, &args)?,
                                         pulse_ast::value::NativeFunctionKind::Async(f) => f(self, &args).await?,
                                     };
                    ...
                    Op::BuildClass => {
                        ...
                        let class_obj = Object::Class(pulse_ast::object::Class {
                            name,
                    ...
                    Op::CreateSharedMemory => {
                        ...
                        let shared_mem = pulse_ast::object::SharedMemory {
                            value: initial_value,
                            locked: false,
                        };
                        ...
                        if let Some(ref shared_heap) = self.shared_heap {
                            let handle = shared_heap.alloc(pulse_ast::object::Object::SharedMemory(shared_mem));
                            ...
                            let handle = self.heap.alloc(pulse_ast::object::Object::SharedMemory(shared_mem));
                            self.push(Value::Obj(handle));
                        }
                        return Ok(VMStatus::Running);
                    }
                    ...
                    Op::ReadSharedMemory => {
                        ...
                        if let pulse_ast::object::Object::SharedMemory(shared_mem) = obj {
                    ...
                    Op::WriteSharedMemory => {
                        ...
                        if let pulse_ast::object::Object::SharedMemory(ref mut shared_mem) = obj {
                    ...
                    Op::AtomicCreate => {
                        ...
                        // Create atomic int
                        let atomic_int = pulse_ast::object::AtomicInt::new(initial);
                        let handle = self.heap.alloc(pulse_ast::object::Object::AtomicInt(atomic_int));
                        self.push(Value::Obj(handle));
                        return Ok(VMStatus::Running);
                    }
                    ...
                    Op::AtomicLoad => {
                        ...
                        if let pulse_ast::object::Object::AtomicInt(atomic_int) = obj {
                    ...
                    Op::AtomicStore => {
                        ...
                        if let pulse_ast::object::Object::AtomicInt(ref atomic_int) = obj {
