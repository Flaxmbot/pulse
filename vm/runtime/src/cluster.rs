...
pub enum ClusterMessage {
    ...
    GossipRequest { node_id: NodeId },
    /// Response with member list
    GossipResponse { node_id: NodeId, members: Vec<Node> },
...
impl ClusterState {
    ...
    pub fn check_timeouts(&mut self, timeout: Duration) -> Vec<MembershipEvent> {
        ...
        let timed_out: Vec<String> = self
            .nodes
            .iter()
            .filter(|(id, node)| id.as_str() != self.node_id.0 && node.is_timed_out(timeout))
            .map(|(id, _)| id.clone())
            .collect();
