use pulse_core::object::{HeapInterface, ObjHandle, Object};
...
impl Heap {
    ...
    pub fn alloc(&mut self, object: Object) -> ObjHandle {
        ...
        let size_estimate = match &object {
            ...
            Object::Regex(_) => 16,
            Object::WebSocket(_) => 16, // Rough estimate - Arc<Regex>
        };
        ...
        if let Some(idx) = self.free_head {
            ...
            if let ObjectEntry::Free { next_free } = self.objects[idx] {
                ...
                self.objects[idx] = ObjectEntry::Allocated {
                    object,
                    marked: false,
                };
            ...
            self.objects.push(ObjectEntry::Allocated {
                object,
                marked: false,
            });
            self.bytes_allocated += size_estimate;
    ...
    pub fn sweep(&mut self) -> usize {
        ...
        for i in 0..self.objects.len() {
            ...
            if is_garbage {
                ...
                self.objects[i] = ObjectEntry::Free {
