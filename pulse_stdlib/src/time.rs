...
use chrono::{DateTime, Local, NaiveDateTime, Timelike, Datelike, NaiveDate};

/// current_timestamp() -> Float
/// Returns the current Unix timestamp in seconds (with microsecond precision)
pub fn current_timestamp_native(_heap: &mut dyn HeapInterface, args: &[Value]) -> PulseResult<Value> {
    if !args.is_empty() {
        return Err(PulseError::RuntimeError("current_timestamp expects 0 arguments".into()));
    }
...
pub fn current_timestamp_millis_native(_heap: &mut dyn HeapInterface, args: &[Value]) -> PulseResult<Value> {
    if !args.is_empty() {
        return Err(PulseError::RuntimeError("current_timestamp_millis expects 0 arguments".into()));
    }
...
pub fn current_timestamp_micros_native(_heap: &mut dyn HeapInterface, args: &[Value]) -> PulseResult<Value> {
    if !args.is_empty() {
        return Err(PulseError::RuntimeError("current_timestamp_micros expects 0 arguments".into()));
    }
...
pub fn sleep_seconds_native<'a>(_heap: &'a mut dyn HeapInterface, args: &'a [Value]) -> Pin<Box<dyn Future<Output = PulseResult<Value>> + Send + 'a>> {
    let args = args.to_vec();
    async move {
        if args.len() != 1 {
            return Err(PulseError::RuntimeError("sleep_seconds expects 1 argument".into()));
        }
        ...
        if secs < 0.0 {
            return Err(PulseError::RuntimeError("sleep_seconds: duration must be non-negative".into()));
        }
...
pub fn now_native(heap: &mut dyn HeapInterface, args: &[Value]) -> PulseResult<Value> {
    ...
    map.insert("millisecond".to_string(), Value::Int(now.timestamp_subsec_millis() as i64));
    ...
    map.insert("weekday".to_string(), Value::Obj(heap.alloc_object(Object::String(weekday_str.to_string()))));
...
pub fn format_time_native(heap: &mut dyn HeapInterface, args: &[Value]) -> PulseResult<Value> {
    if args.len() != 2 {
        return Err(PulseError::RuntimeError("format_time expects 2 arguments".into()));
    }
...
pub fn parse_time_native(heap: &mut dyn HeapInterface, args: &[Value]) -> PulseResult<Value> {
    if args.len() != 2 {
        return Err(PulseError::RuntimeError("parse_time expects 2 arguments".into()));
    }
...
fn parse_time_parsed(time_str: &str, format: &str) -> PulseResult<NaiveDateTime> {
    ...
    if format == "%H:%M:%S" {
        ...
        return Ok(today.and_hms_opt(time.hour(), time.minute(), time.second()).unwrap());
    }
    
    Err(PulseError::RuntimeError(format!("Unsupported format: {}", format)))
}
...
pub fn duration_create_native(heap: &mut dyn HeapInterface, args: &[Value]) -> PulseResult<Value> {
    if args.len() != 1 {
        return Err(PulseError::RuntimeError("duration_create expects 1 argument".into()));
    }
    ...
    map.insert("milliseconds".to_string(), Value::Int(duration.as_millis() as i64));
    map.insert("microseconds".to_string(), Value::Int(duration.as_micros() as i64));
...
pub fn duration_add_native(heap: &mut dyn HeapInterface, args: &[Value]) -> PulseResult<Value> {
    if args.len() != 2 {
        return Err(PulseError::RuntimeError("duration_add expects 2 arguments".into()));
    }
                ...
                if let Some(Object::Map(m)) = heap.get_object(*h) {
                    if let Some(v) = m.get("seconds") {
                        match v {
                            Value::Float(f) => Ok(*f),
                            Value::Int(i) => Ok(*i as f64),
                            _ => Err(PulseError::RuntimeError("Duration must have seconds field".into()))
                        }
                    } else {
                        Err(PulseError::RuntimeError("Duration must have seconds field".into()))
                    }
                } else {
                    Err(PulseError::TypeMismatch { expected: "map".into(), got: "object".into() })
                }
            }
            _ => Err(PulseError::TypeMismatch { expected: "number or map".into(), got: arg.type_name() })
        }
    };
...
pub fn measure_time_native<'a>(heap: &'a mut dyn HeapInterface, args: &'a [Value]) -> Pin<Box<dyn Future<Output = PulseResult<Value>> + Send + 'a>> {
    let args = args.to_vec();
    async move {
        if args.len() != 1 {
            return Err(PulseError::RuntimeError("measure_time expects 1 argument".into()));
        }
        ...
        let _ = args[0].clone(); // Use the argument to avoid unused warning
        let elapsed = start.elapsed();
        
        let mut map: HashMap<String, Value> = HashMap::new();
        map.insert("seconds".to_string(), Value::Float(elapsed.as_secs_f64()));
        map.insert("milliseconds".to_string(), Value::Int(elapsed.as_millis() as i64));
        map.insert("microseconds".to_string(), Value::Int(elapsed.as_micros() as i64));
...
pub fn unix_to_datetime_native(heap: &mut dyn HeapInterface, args: &[Value]) -> PulseResult<Value> {
    if args.len() != 1 {
        return Err(PulseError::RuntimeError("unix_to_datetime expects 1 argument".into()));
    }
...
pub fn datetime_to_unix_native(heap: &mut dyn HeapInterface, args: &[Value]) -> PulseResult<Value> {
    if args.len() != 1 {
        return Err(PulseError::RuntimeError("datetime_to_unix expects 1 argument".into()));
    }
