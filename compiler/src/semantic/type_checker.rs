    ...
    fn infer_stmt(&mut self, stmt: &Stmt) -> TypeCheckResult<Type> {
        match stmt {
            ...
            Stmt::Send(target, message) => {
                // Main execution acts as the root actor in Pulse, so sending messages
                // globally is allowed. Disabling strict effect check for this.

                let target_ty = self.infer_expr(target)?;
                // Disabling strict unification with `Pid` for now to support string/named actors
